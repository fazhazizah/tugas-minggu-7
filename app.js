  ...
  rl.question('Masukan Nama Folder: ', (folderName) => {
    ...
    if (!fs.existsSync(folderPath)) {
      fs.mkdir(folderPath, (err) => {
        if (err) throw err;
        console.log(`Folder ${folderName} berhasil dibuat!`); // Fixed
      });
    } else {
      console.log(`Folder ${folderName} sudah ada.`); // Fixed
    }
    rl.close();
  });
};
  ...
  fs.readdir(rootDir, (err, items) => {
    ...
    rl.question('Masukkan Nama Folder untuk membuat file: ', (folderName) => {
      ...
      if (!fs.existsSync(targetFolder)) {
        console.log(`Folder ${selectedFolder} tidak ditemukan.`); // Fixed
        rl.close();
        return;
      }
      ...
      rl.question('Masukkan Nama File : ', (fileName) => {
        ...
        fs.writeFile(filePath, '', (err) => {
          if (err) throw err;
          console.log(`File ${fileName} berhasil dibuat di folder ${selectedFolder}!`); // Fixed
          rl.close();
        });
      });
    });
  ...
  fs.readdir(folderPath, (err, files) => {
    ...
    files.forEach((file) => {
      ...
      if (['jpg', 'jpeg', 'png'].includes(ext)) {
        ...
        console.log(`File ${file} tidak termasuk jenis image atau text, dilewati.`); // Fixed
        return;
      }
      ...
      fs.rename(oldPath, newPath, (err) => {
        if (err) throw err;
        console.log(`File ${file} berhasil dipindahkan ke folder ${path.basename(targetFolder)}`); // Fixed
      });
    });
  });
};
  ...
  rl.question('Masukan Nama Folder: ', (folderName) => {
    ...
    fs.readdir(folderPath, (err, files) => {
      ...
      const fileDetails = files.map((file) => {
        ...
        const formattedDate = `${stats.birthtime.getFullYear()}-${String(stats.birthtime.getMonth() + 1).padStart(2, '0')}-${String(stats.birthtime.getDate()).padStart(2, '0')}`; // Fixed
      ...
      console.log(`Berhasil menampilkan isi dari folder ${folderName}:`, fileDetails); // Fixed
      rl.close();
    });
  });
};
    ...
    findFile(__dirname)
      .then((filePath) => {
        if (filePath) {
          ...
          console.log(`File ${fileName} tidak ditemukan di dalam proyek.`); // Fixed
          rl.close();
        }
      })
      .catch((err) => {
